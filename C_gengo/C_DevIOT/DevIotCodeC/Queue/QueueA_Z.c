...
/*
Queue(Hàng đợi) ís a data structure to save object with 
feature: FIFO (First in first out)
In Queue, only can add eles to the edge of queue and 
the same as when delete eles.
Some feature of Queue:
    EnQueue: Add ele to rear of Queue;
    DeQueue: Delete ele from first Queue. if emmpty, have error. 
    IsEmpty: Check Queue empty.
    Front: take value of first ele of Queue (not change Queue).
Need some variable to set a queue:
    queue[]: 1D array simulates queue
    arraySize: max elements can be stored in queue
    front: the first element of queue
    rear: the end element of queue
*/
